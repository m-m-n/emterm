...
- **コアターミナル**
  ...
  - BCE（背景色消去）対応
  - 拡張OSC対応: カラーパレット設定・照会（応答付き）とリセット（OSC 4/10/11/12、104/110/111/112）、デスクトップ通知/プログレス（OSC 9）、マウスカーソル形状（OSC 22）、iTerm2互換 OSC 1337 File / SetUserVar
  - Synchronized Output（DECプライベートモード2026）と DECRPM による対応検出
  - スクロール領域のスクロールバック: DECSTBMによる非全画面スクロール領域から外れた行を破棄せずスクロールバックに保持（Ghosttyと同一挙動、設定で切替可能・デフォルトON）
...
- **リッチコンテンツ表示**
  ...
  - Markdown フロントマター: 先頭のYAML/TOML/JSONフロントマターを検出して本文から分離し、折りたたみ可能なメタデータブロック（展開可能なツリー表示）として表示。パース失敗時はエラー表示と生テキストを表示
...
- **ターミナルマルチプレクサ**
  ...
  - 設定 > エージェントカテゴリの通知トグル: ターン終了（done）・質問待ち（blocked）・可視ペイン通知（フォーカス中ウィンドウのアクティブタブでも通知）を個別にON/OFF可能（デフォルトすべてON）
...
- **入力・IME**
  ...
  - Shift+Enter動作設定: そのまま送信 / Alt+Enterとして送信（デフォルト） / LFとして送信、AIインターフェースでのマルチライン入力に対応
  - 単語・行選択ドラッグ: ダブルクリック（単語）またはトリプルクリック（行）の後にドラッグしても、最初にクリックした単語/行がドラッグ中ずっと選択に含まれ続ける。ドラッグ中にホイール操作や別ボタンの押下が挟まっても、またボタンを押したままフォーカスを失っても、ドラッグは継続する
  - Enter送信時またはコピー時にマウス選択がクリアされ、TUIの行書き換え（Claude Codeなど）による選択ハイライトの残留を防ぐ
  - 包括的な特殊キーマッピング（Ctrl+記号、修飾キー付き矢印キー・ファンクションキー、Shift+Tab）
  - Alternate Scroll モード（DECSET 1007）: オルタネートスクリーンのアプリではマウスホイールを矢印キーとして送信（デフォルトON、設定で切替可能）
  - DEC プライベートモード 1000/1002/1003 のマウスレポート（X10 / SGR(1006) エンコーディング）: vim・tmux・htop・Claude Code などの TUI/フルスクリーンアプリがクリック・ドラッグ・ホイール操作を受け取れる。Shift を押すとテキスト選択・リンクオープン・ミドルクリックペースト・スクロールバック/Alternate Scroll といったeMtermのローカル動作を強制できる。タブバー・ステータスバー・スクロールバー・muxサイドバー・ウィンドウ端などeMterm自身のUIは常にローカル動作を維持する

- **ナビゲーション**
  - OSC 133 セマンティックプロンプトジャンプ（Ctrl+上/下）
  - マッチハイライト付きインクリメンタルテキスト検索（Ctrl+F）
  - コマンド出力の折りたたみ（展開/折りたたみ）
  - スクロール固定: スクロールバック閲覧中は新しい出力で表示が流れない。キー入力で自動的に最新表示（live tail）へ復帰
  - ファイルパスのCtrl+クリックでエディタ起動（ホバー時のみ下線表示）
  - URLのCtrl+クリックでブラウザ起動（ホバー時のみ下線表示）
  - ターミナル領域・タブ・タブバーへの右クリックコンテキストメニュー
  - OSC 8ハイパーリンク: Ctrl+ホバーでリンクセルに下線表示、Ctrl+クリックで開く。正規表現によるURL検出と異なりオルタネートスクリーン内でもクリック可能
...
- **設定・外観**
  ...
  - フォント設定（プライマリ・CJK/セカンダリ・カラー/モノクロ絵文字を個別指定）とシステムフォントピッカー（クリアボタン付き）
  - 4段階のフォント解決順序: 設定パス > ユーザーフォントディレクトリ（ファイル配置による差し替え） > システムフォント > バンドルフォント
  - 設定パネル用UIフォント設定
  - 設定・Markdown・JSON/YAMLビューアーウィンドウは最大化状態で起動。全ウィンドウが1つのドックアイコンにグループ化（`WM_CLASS` / `app_id` = `emterm`）
  - ターミナルプロファイル: シェル・引数・環境変数・作業ディレクトリを持つ名前付きシェル設定
  - SSH接続管理: sshコマンド自動検出、~/.ssh/configからのインポート、接続エントリのCRUD
  - SFTPファイルアップロード: SSHタブへのドラッグ&ドロップでファイル転送（非SSHタブはファイルパスをペースト）（Linux Waylandでは現在利用不可 — [既知の問題](#既知の問題)参照）
  - WSLディストリビューション検出・インポート・プロファイル統合（Windowsのみ）
  - カーソル形状、スクロールバー、透明度、行高、スクロールバック行数、シェルなど各種設定
  - 全キーボードショートカット設定可能
  - モーダルダイアログ（ネイティブ・WebView共通）はMaterial Design 3ベースの単一デザインシステムに統一され、レイアウト・キーボード操作・配色ルールが一貫

- **通知**
  - バックグラウンドタブで新しい出力やプロセスイベントがあると、タブにアクティビティドットを表示
  - ウィンドウが非フォーカス時のOSデスクトップ通知（設定で切替可能）
  - 高頻度出力時の通知スロットリングによるスパム防止
  - 通知のタイトル・本文はマークアップ対応の通知サーバーに送る前にエスケープされ、ログファイルにも書き出されない
  - 通知の送出は専用のバックグラウンドスレッドで行われるため、OS通知デーモンが遅い・ハングしていてもタイプ入力のレイテンシに影響しない。キュー（容量8）が満杯になると以降の通知はドロップされる
  - `apt`/`dpkg`等のパッケージ更新でバイナリが差し替わった際、実行中のバイナリと一致しなくなったことを検知して再起動を促すトースト表示（Linuxのみ）
  - Windows: `.exe` に埋め込まれたアプリケーションアイコンを全ウィンドウに表示。シェルの自然終了（`exit`、Ctrl+D）で確実にタブが閉じる
...
### Linux Waylandでファイルのドラッグ&ドロップが動作しない

LinuxではeMtermはデフォルトでWaylandネイティブクライアントとして動作します。同梱のwinit（`0.31.0-beta.2`）にはWayland向けのドラッグ&ドロップ実装がまだ含まれていないため、Wayland上ではターミナルウィンドウへのファイルドロップ（SFTPアップロード / パスのペースト）が反応しません。実装はwinitのmasterブランチには取り込み済みで、次のリリース公開後にwinitを更新することで解消される見込みです。

回避策: ドラッグ&ドロップが必要な場合はX11バックエンドで起動してください：
