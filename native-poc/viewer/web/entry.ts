...
import { MarkdownRenderer } from "../../../src/markdown/renderer.ts";
import { OutlinePanel } from "../../../src/markdown/outline.ts";
import { MermaidRenderer } from "../../../src/markdown/mermaid-renderer.ts";
import {
	applyMarkdownColorTheme,
	applyMarkdownSettings,
} from "../../../src/settings/settings-applier.ts";

import type { MarkdownFormat } from "../../../src/markdown/types.ts";
import type { UiTheme, UiThemePreset } from "../../../src/settings/types.ts";
...
import "../../../src/markdown/fullscreen.css";
import "../../../src/markdown/outline.css";
 ...
 */
export function renderPayload(root: HTMLElement, payload: ViewerPayload): HTMLElement {
	...
	for (const img of content.querySelectorAll<HTMLImageElement>("img[data-local-src]")) {
		const localPath = img.dataset.localSrc ?? img.getAttribute("data-local-src") ?? "";
		if (!localPath || localPath.startsWith("data:") || /^[a-zA-Z][a-zA-Z0-9+\-.]*:\/\//.test(localPath)) {
			continue;
		}
