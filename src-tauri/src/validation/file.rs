...
pub fn validate_file_path(path: &Path) -> Result<PathBuf, CommandError> {
    ...
    // Return the validated path (follow symlinks by using canonicalize is optional)
    // For now, we just return the path as-is to avoid canonicalize issues with non-existent targets
    Ok(path.to_owned())
}
...
pub fn open_and_validate_file(path: &Path, max_size: u64) -> Result<(File, PathBuf), CommandError> {
    // Open file (this will fail if file doesn't exist or isn't readable)
    ...
    // Get metadata from the open file handle (no TOCTOU)
    let metadata = file.metadata()?;

    // Check it's a file (not directory or symlink to directory)
    if !metadata.is_file() {
        return Err(CommandError::NotAFile(path.to_owned()));
    }
