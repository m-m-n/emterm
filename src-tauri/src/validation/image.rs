...
pub fn validate_image_format(file: &mut File) -> Result<ImageFormat, CommandError> {
    // Read first few bytes to detect format via magic bytes
    ...
    // Rewind file to beginning for subsequent reads
    use std::io::Seek;
