...
fn run_with_click(items: &[SidebarEntry], placement: Placement, click_pos: Pos2) -> SidebarOutcome {
    let ctx = egui::Context::default();
...
fn ac3_overlay_row_insets_hold_on_post_resize_frame() {
    ...
    let width2 = 300.0;
    let card = draw_overlay_two_frames_and_capture_card(&items, screen1, width1, screen2, width2);
    let rows = LAST_ROW_RECTS.with(|c| c.borrow().clone());
    assert_eq!(rows.len(), 1);
    assert!(
        (rows[0].right() - (card.rect.right() - PANEL_PAD_HORIZONTAL)).abs() < 0.5,
...
fn ellipsized_layout_truncates_when_overflowing_available_width() {
    ...
    let _ = ctx.run(input, |ctx| {
        egui::CentralPanel::default().show(ctx, |ui| {
            ...
            let truncated = ui.fonts(|f| layout_ellipsized(f, long, &font, Color32::WHITE, max_w));
...
fn ellipsized_layout_leaves_short_text_untouched() {
    ...
    let _ = ctx.run(input, |ctx| {
        egui::CentralPanel::default().show(ctx, |ui| {
            let font = FontId::proportional(NAME_FONT_SIZE);
            let full =
                ui.fonts(|f| f.layout_no_wrap("short".to_string(), font.clone(), Color32::WHITE));
            let result = ui.fonts(|f| layout_ellipsized(f, "short", &font, Color32::WHITE, 500.0));
...
fn ac1_ac2_click_at_the_row_s_former_icon_region_reports_a_window_switch() {
    ...
    let former_icon_region = egui::pos2(row.right() - ROW_HORIZONTAL_PAD - 10.0, row.center().y);
