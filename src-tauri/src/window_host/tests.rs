...
/// AC-3 (task0007). The two production call sites that hold live
/// `host.mouse_report_held` values — `run_button_decision`'s button path
/// and `handle_mouse_wheel`'s wheel path — must keep passing that live
/// value into the held-aware seam entry points
/// (`mouse_report::apply_outcome_with_held` /
/// `mouse_report::apply_wheel_report_step`) as an actual call argument,
/// not merely somewhere in the same function body. A source-scan test that
/// only checks both needles appear anywhere in the whole file (like
/// `pointer_routing_handlers_hold_no_decision_only_delegate_to_the_seam`
/// above) stays green even if one call site regresses onto the
/// held-unaware `mouse_report::apply_outcome` (which defaults
/// `HeldButtons` to `HeldButtons::default()`) while the other still names
/// `host.mouse_report_held` elsewhere in the file — losing the held left
/// gesture-slot's ownership across a second button press mid-drag, or
/// across a wheel event mid-drag, and re-latching `host.dragging`. This
/// test isolates each function body by brace depth and asserts
/// `host.mouse_report_held` appears inside the actual argument list of the
/// held-aware call within THAT function, so a regression in either call
/// site is pinned independently of the other.
#[test]
fn button_and_wheel_paths_pass_live_held_buttons_into_the_held_aware_seam_entry_points() {
    let src = include_str!("pointer_routing.rs");

    /// Extracts the `{ ... }` body of the function whose signature
    /// contains `fn <name>(` in `src`, by counting brace depth from the
    /// opening `{` after the signature — NOT by scanning to the next
    /// `fn`, which would silently swallow (or stop short of) the body if
    /// a helper function were later inserted nearby. Skips `//` line
    /// comments, `/* */` block comments, and `"..."` / `'.'` literals so
    /// braces inside them (e.g. the `"{other:?}"` format strings already
    /// present in both target functions) never desync the depth count.
    /// Neither target function contains a raw string literal today; if
    /// one is added later and this assertion starts failing on a
    /// brace-count desync, that's the signal to extend the skip logic
    /// rather than deleting the test.
    fn extract_fn_body<'a>(src: &'a str, name: &str) -> &'a str {
        let sig_needle = format!("fn {name}(");
        let sig_at = src
            .find(&sig_needle)
            .unwrap_or_else(|| panic!("expected `fn {name}(` in pointer_routing.rs"));
        let open_brace = src[sig_at..]
            .find('{')
            .map(|i| sig_at + i)
            .unwrap_or_else(|| panic!("expected `{{` after `fn {name}(` signature"));
        ...
        let end =
            body_end.unwrap_or_else(|| panic!("brace depth never returned to 0 for `fn {name}`"));
        &src[open_brace..end]
    }

    /// Returns the whitespace-normalized argument-list text between the
    /// matching parens of the first call to `call_prefix` (e.g.
    /// `"mouse_report::apply_outcome_with_held("`, including its opening
    /// paren) found in `body`.
    fn call_args(body: &str, call_prefix: &str) -> String {
        let start = body
            .find(call_prefix)
            .unwrap_or_else(|| panic!("expected `{call_prefix}` inside the function body"));
        let args_start = start + call_prefix.len();
        let bytes = body.as_bytes();
        let mut depth: i32 = 1; // call_prefix's own opening '(' already consumed
        ...
        let end = end.unwrap_or_else(|| panic!("unbalanced parens after `{call_prefix}`"));
        body[args_start..end]
            .split_whitespace()
            .collect::<Vec<_>>()
            .join(" ")
    ...
    let button_body = extract_fn_body(src, "run_button_decision");
    let button_args = call_args(button_body, "mouse_report::apply_outcome_with_held(");
    assert!(
        button_args.contains("host.mouse_report_held"),
        "AC-3 (task0007): run_button_decision must pass the live `host.mouse_report_held` \
         value as an actual argument of its `mouse_report::apply_outcome_with_held(...)` call \
         — if this call site regresses onto the held-unaware `mouse_report::apply_outcome` (or \
         hardcodes a default), a second button's press mid-left-drag loses the held left \
         gesture slot's ownership and `host.dragging` latches. Got args: `{button_args}`"
    );

    let wheel_body = extract_fn_body(src, "handle_mouse_wheel");
    let wheel_args = call_args(wheel_body, "mouse_report::apply_wheel_report_step(");
    assert!(
        wheel_args.contains("host.mouse_report_held"),
        "AC-3 (task0007): handle_mouse_wheel must pass the live `host.mouse_report_held` value \
         as an actual argument of its `mouse_report::apply_wheel_report_step(...)` call — the \
         same held-left-gesture-slot-loss / `host.dragging` latch regression as the button \
         path, reached via a wheel event mid-drag instead of a second button press. Got args: \
         `{wheel_args}`"
    );
}
