...
mod anchor_scan {
    ...
    /// closes two levels (EC-3). anchor-scan-turbofish-arrow/task0001: a
    ...
    /// (e.g. `fn(u8) -> u8`, at any nesting depth) — it is skipped
    /// without changing depth, never counted as a close. Any other `-`
    /// before a `>` (separated by whitespace or a comment, since the
    /// scanner already discarded those tokens and byte-adjacency no
    /// longer holds; or with something other than `-` — such as a
    /// numeric literal for a negative const generic — immediately
    /// before it) closes the list as usual. Tokens other than `<` / `>`
    /// are otherwise skipped. Returns the index of the balancing `>`,
    /// or `None` when the walk reaches `end` without finding it — the
    /// walk never goes past the body end.
    ...
    pub(super) fn locate_call(
        ...
        //     return-type arrow inside the generic argument (e.g.
        //     `fn(u8) -> u8`) does not prematurely close the list —
        //     `find_balancing_angle_close` skips the arrow's own `>`
        //     (anchor-scan-turbofish-arrow/task0001). A turbofish-shaped
