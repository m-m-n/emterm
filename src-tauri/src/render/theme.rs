...
/// (FR5).
///
/// `terminator == OscTerminator::Unterminated` (task0001's SC-1: the
/// request was cut short by something other than a real BEL/ST terminator)
/// produces `None` — never a response — rather than inventing an
/// unspecified terminator for a request that was never validly delimited
/// in the first place (REQUIREMENTS.md FR5 only defines BEL and ST forms).
/// This is task0002's decision on SC-4's "query side" ownership, matching
/// the FR10 inertness contract: a malformed request produces no response
/// bytes and no state change.
fn format_osc_color_response(
    ...
    let terminator_bytes: &[u8] = match terminator {
        OscTerminator::Bel => &[0x07],
        OscTerminator::St => &[0x1b, b'\\'],
        OscTerminator::Unterminated => return None,
    };
    ...
    fn apply_default_color_set(
        ...
        for (offset, spec) in data.split(';').enumerate() {
            ...
            if trimmed == "?" {
                ...
                if let Some(resp) = format_osc_color_response(target, None, rgb, terminator) {
                    outcome.responses.push(resp);
                }
                continue;
            }
...
mod tests {
    ...
    fn apply_osc_4_chained_entries() {
        let mut t = Theme::default();
        assert!(
            t.apply_osc(4, "1;rgb:10/00/00;200;rgb:00/aa/00", OscTerminator::Bel)
                .changed
        );
    ...
    fn apply_osc_4_invalid_pair_skipped() {
        ...
        assert!(
            t.apply_osc(4, "999;rgb:00/00/00;7;rgb:ff/ff/ff", OscTerminator::Bel)
                .changed
        );
    ...
    fn apply_osc_104_empty_resets_overlay_and_restores_palette16_from_scheme() {
        ...
        assert!(t.apply_osc(4, "5;rgb:11/22/33", OscTerminator::Bel).changed);
        assert!(
            t.apply_osc(4, "200;rgb:44/55/66", OscTerminator::Bel)
                .changed
        );
    ...
    fn apply_osc_104_indexed_resets_only_listed_overlay_and_palette16() {
        ...
        let mut t = Theme::default();
        assert!(t.apply_osc(4, "5;rgb:11/22/33", OscTerminator::Bel).changed);
        assert!(t.apply_osc(4, "6;rgb:44/55/66", OscTerminator::Bel).changed);
        let scheme_5 = t.scheme_palette16[5];
    ...
    fn apply_osc_104_indexed_reset_at_or_above_16_clears_overlay_only() {
        ...
        assert!(t.apply_osc(104, "200", OscTerminator::Bel).changed);
    ...
    fn apply_osc_104_indexed_reports_change_when_only_that_indexs_array_entry_diverges() {
        ...
        let mut t = Theme::default();
        t.palette16[3] = Rgb(9, 9, 9);
        assert!(t.apply_osc(104, "3", OscTerminator::Bel).changed);
        assert_eq!(t.palette16[3], t.scheme_palette16[3]);
    }
    ...
    fn apply_osc_104_post_reset_resolution_matches_scheme_for_0_15_and_cube_for_16_255() {
        ...
        assert!(
            theme
                .apply_osc(4, "3;rgb:11/22/33", OscTerminator::Bel)
                .changed
        );
        assert!(
            theme
                .apply_osc(4, "100;rgb:44/55/66", OscTerminator::Bel)
                .changed
        );

        assert!(theme.apply_osc(104, "", OscTerminator::Bel).changed);
    ...
    fn apply_osc_set_sequences_never_change_scheme_mirrors() {
        ...
        assert!(t.apply_osc(4, "3;rgb:11/22/33", OscTerminator::Bel).changed);
        assert!(t.apply_osc(10, "rgb:44/55/66", OscTerminator::Bel).changed);
        assert!(t.apply_osc(11, "rgb:77/88/99", OscTerminator::Bel).changed);
        assert!(t.apply_osc(12, "rgb:aa/bb/cc", OscTerminator::Bel).changed);
    ...
    fn from_settings_osc110_osc111_reset_to_active_scheme_not_hardcoded_default() {
        ...
        assert!(
            theme
                .apply_osc(10, "rgb:01/02/03", OscTerminator::Bel)
                .changed
        );
        assert!(
            theme
                .apply_osc(11, "rgb:04/05/06", OscTerminator::Bel)
                .changed
        );

        assert!(theme.apply_osc(110, "", OscTerminator::Bel).changed);
        assert!(theme.apply_osc(111, "", OscTerminator::Bel).changed);
    ...
    fn from_settings_osc12_then_osc112_resets_to_active_scheme_cursor() {
        ...
        assert!(
            theme
                .apply_osc(12, "rgb:aa/bb/cc", OscTerminator::Bel)
                .changed
        );
    ...
    fn apply_osc_10_query_answers_with_live_fg_and_causes_no_change() {
        ...
        assert!(
            !outcome.changed,
            "AC-6: answering a query is never a visible change"
        );
        assert_eq!(
            outcome.responses,
    ...
    fn apply_osc_11_query_answers_with_live_bg_and_causes_no_change() {
        ...
        assert_eq!(
            outcome.responses,
            vec![b"\x1b]11;rgb:4444/5555/6666\x07".to_vec()]
        );
        assert_eq!(t.bg, Rgb(0x44, 0x55, 0x66));
    }
    ...
    fn apply_osc_12_query_answers_with_live_cursor_fg_and_causes_no_change() {
        ...
        assert_eq!(
            outcome.responses,
            vec![b"\x1b]12;rgb:aaaa/bbbb/cccc\x07".to_vec()]
        );
        assert_eq!(t.cursor_fg, Rgb(0xaa, 0xbb, 0xcc));
    }
    ...
    fn apply_osc_10_chained_four_queries_produces_no_fourth_response() {
        ...
        assert_eq!(
            ...
            3,
            "the 4th query element (target 13) is dropped, matching the existing set-path break at >12"
        );
    }
    ...
    fn apply_osc_4_mixed_set_query_set_applies_sets_and_answers_query_in_order() {
        let mut t = Theme::default();
        let outcome = t.apply_osc(4, "5;rgb:01/02/03;7;?;9;rgb:04/05/06", OscTerminator::Bel);
        assert!(outcome.changed);
