...
- If the path starts with `~` (home-directory shorthand), resolve it to an
  ...
  `emterm markdown -- '/absolute/path/notes.md'`. This keeps one
  invariant with no special case: no byte derived from an untrusted path
  ever appears outside the single quotes.
