...
- **FR2 — Popup overlay open**: Clicking the Spread button constructs a `.mermaid-popup-overlay` element (positioned `fixed`, `inset: 0`, `z-index: 2000` so it sits above `.markdown-fullscreen-overlay` at 1000 and below `.dialog-shell` at 3000), appends it to `document.body`, clones the diagram SVG (`.mermaid-diagram > svg`) via `cloneNode(true)`, and centers the clone. **Clone sizing normalization**: Mermaid renders with `useMaxWidth: true`, so the source SVG carries `width="100%"` and an inline `style="max-width: <n>px"` which the clone inherits; a stylesheet rule cannot override inline styles, and merely *removing* the sizing attributes is insufficient (an SVG without `width`/`height` resolves its base box from the available layout width, not from the `viewBox`, which makes the fit effectively width-based and clips tall diagrams). Therefore after cloning, set explicit `width` / `height` **attributes** on the clone from the `viewBox`-derived intrinsic size (px), clear the inline `width` / `height` styles, and set inline `max-width` / `max-height` to `none`, so the clone's untransformed base box equals the intrinsic size in both dimensions and `k` truly fits both width and height. On construction, compute `stageWidth = window.innerWidth * 0.8`, `stageHeight = window.innerHeight * 0.8`, derive the SVG's intrinsic size from its `viewBox` (or `getBoundingClientRect()`), and set the initial fit factor `k` such that `k = min(stageWidth / svgWidth, stageHeight / svgHeight)`. Store `k` as the reference; the exposed `scale` variable starts at `1.0` and the applied CSS transform is `translate(panX, panY) scale(scale * k)`. The cloned SVG carries an opaque background matching the Mermaid dark-theme canvas (`#2d2d2d`) with rounded corners and a drop shadow, so the diagram's area is visually distinct from the click-to-close overlay background (prevents accidental closes from clicks the user believed were inside the diagram).
...
- **FR4 — Pan controls**: `mousedown` on the stage sets `dragging = true`; window-level `mousemove` while dragging updates `panX += event.movementX` / `panY += event.movementY`; window-level `mouseup` sets `dragging = false` (window-level listeners cover releases outside the stage, which is why no pointer capture or stage-`mouseleave` handling is needed). As a stuck-drag guard, `window` `blur` while dragging also sets `dragging = false`. Cursor is `grab` normally, `grabbing` during drag. **Arrow-key pan**: `ArrowUp` / `ArrowDown` / `ArrowLeft` / `ArrowRight` on the overlay pan the view by 40px per press with scroll-direction semantics (`ArrowRight` reveals content to the right, i.e. `panX -= 40`; `ArrowDown` reveals content below, i.e. `panY -= 40`), each with `preventDefault()`.
...
- **FR6 — Close**: The popup listens for close triggers in three ways:
  ...
  **Native ESC-close suppression**: DOM-level `stopPropagation()` cannot reach the viewer's *native* window-level close handler (`webview_host` `close_on_esc_q`: GTK `key_press_event` on Linux, winit `KeyboardInput` on Windows), which otherwise closes the entire viewer on the same ESC press. The popup therefore sends a reserved host IPC message on open (`__emterm_host:esc-guard:on`) and on close (`__emterm_host:esc-guard:off`) via `window.ipc.postMessage` (best-effort — silently skipped when `window.ipc` is unavailable, e.g. in unit tests). While the guard is on, the native handler suppresses ESC / `q` / `Q` window-close. `webview_host` registers its wry IPC handler whenever `close_on_esc_q` is true (even with no user-level IPC configured) so `window.ipc` exists in the viewer. The two reserved messages (`__emterm_host:esc-guard:on` / `__emterm_host:esc-guard:off`, exact match) are consumed **inside the IPC callback itself** — the guard toggles synchronously at message arrival and reserved messages are never enqueued into the host's event channel — so guard application cannot lose a race against the next key event and reserved traffic cannot grow the channel. Any other body — including other `__emterm_host:`-prefixed strings — is forwarded to the user-level handler as usual (on Linux via a drain capped at 64 messages per GTK iteration so an IPC flood cannot starve UI event processing; on Windows the guard is an `Arc<AtomicBool>` because the IPC callback may run off the main thread).
...
- [ ] **TS-19 — Clone sizing normalization**: The cloned SVG in the stage carries explicit `width` / `height` attributes equal to the `viewBox`-derived intrinsic size (px) and inline `max-width` / `max-height` of `none`, so the fit factor applies to the intrinsic size in both dimensions
...
- [ ] **TS-21 — Arrow-key pan**: With the popup open, `ArrowRight` changes `panX` by −40 (reveals content to the right), `ArrowLeft` by +40, `ArrowDown` changes `panY` by −40, `ArrowUp` by +40; each keydown is `preventDefault()`ed
- [ ] **TS-22 — ESC-guard IPC**: With a mocked `window.ipc`, opening the popup posts `__emterm_host:esc-guard:on` and closing it posts `__emterm_host:esc-guard:off`; when `window.ipc` is absent, open/close complete without throwing
- [ ] **TS-23 (Rust) — Host control message parsing**: `webview_host` parses `__emterm_host:esc-guard:on` / `:off` as guard toggles and rejects non-reserved bodies (forwarded to user IPC)
