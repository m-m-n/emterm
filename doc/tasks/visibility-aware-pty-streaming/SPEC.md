...
- **FR8 — Visible 復帰時のスナップショット送信 (非 mux):** backend は復帰時に `build_session_snapshot(session_id)` (`b"\x1b[H\x1b[2J"` プレフィックス + shadow parser の `contents_formatted()` + 画像/Markdown raw passthrough) を生成し、Channel へ 1 メッセージで送信する。送信後 raw passthrough をクリアする。
- **FR9 — Visible 復帰時のスナップショット送信 (mux):** daemon は復帰時に各ペインについて、(1) `output_target` の lock を取った状態で既存 `build_shadow_parser_snapshot(shadow_parser)` + 画像/Markdown raw passthrough バッファを連結した `PtyOutput` を `pane_output_tx` に enqueue し、(2) raw passthrough を `clear()` し、(3) `output_target` を `Connected(pane_output_tx)` に戻す、の順で実施する。`(1)→(3)` を同一 pane lock 内で行うことで、Connected 切替後の reader chunk が snapshot より先に流れるレースを防ぐ。reader thread 側は (3) 後の次回 batch から通常 streaming へ戻る。
...
- **FR12 — Backend からの再構築不要保証:** スナップショットは self-contained な ANSI バイト列であり、frontend WASM grid は受信したスナップショットを通常の `process_pty_data` 経路で消費する。frontend 側に専用デコーダを追加しない。
- **FR13 — mux 既存 detach との共存 (確定):** daemon に「クライアント (GUI) 単位の visible 状態」を保持する。具体的な保持方式は実装中決定とし、現実の `connection.rs` 構造に合わせて以下のいずれかを採用する:
  ...
  - reattach 時に hidden だった場合: `output_target` を `Detached` のまま維持し、snapshot 送信はせず ring buffer に蓄積継続。次回 visible 復帰時に snapshot を送る。
