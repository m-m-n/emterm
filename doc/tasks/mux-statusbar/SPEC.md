...
- **FR3: Command execution** — Execute only registered executables (no arguments, no shell). Expand `~` to home directory. Working directory is fixed to the user's home directory. Timeout after 5 seconds (kill process, retain previous value). Suppress stderr. For multi-line stdout, use only the first line (trimmed).
