...
Other locale entries under `settings.mux.*` (title, general, prefix, statusPosition, keybinds, and the retained keybind labels) stay. (The `settings.mux.mouse` label was removed together with the mux mouse setting in a later task.)
