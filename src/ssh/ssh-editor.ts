...
export function showSshEditor(options: SshEditorOptions): () => void {
	const connection: SshConnection = options.connection
		? { ...options.connection, ssh_options: [...(options.connection.ssh_options || [])] }
		: { name: "", hostname: "", port: 22, username: "", identity_file: "", ssh_options: [] };
	...
	form.addEventListener("submit", async (e) => {
		...
		if (identityFile) {
			try {
				const exists = await invoke<boolean>("validate_identity_file", { path: identityFile });
				if (!exists) {
...
function addCommittedRow(container: HTMLElement, key: string, value: string): void {
...
function collectSshOptions(container: HTMLElement): SshOption[] {
	...
	for (const row of container.querySelectorAll(".ssh-option-row")) {
		...
		const valueInput = row.querySelector(".ssh-option-value") as HTMLInputElement;
