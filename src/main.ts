...
import { showPasteDialog } from "./clipboard/dialog";
import { ClipboardManager } from "./clipboard/manager";
import { sendTextInChunks } from "./clipboard/paste";
import {
	keyEventToBytes,
...
import { coordsToGrid } from "./selection/coords";
...
let isSelecting: boolean = false; // Track if mouse is currently being dragged for selection
...
async function initTerminal(): Promise<void> {
  ...
  `;
	...
	if ("EditContext" in window) {
		...
		if (IME_DEBUG)
			console.log(
				"[IME] EditContext API not available, using textarea fallback",
			);
		...
		// pointer-events: none allows mouse events to pass through to terminal
	...
	// Initialize selection and clipboard managers
	...
	if (imeInput) {
		terminalClickHandler = (e) => {
			...
			if (IME_DEBUG)
				console.log("[IME Debug] terminal mousedown, focusing textarea");
			if (imeInput) {
				imeInput.focus();
				...
				setTimeout(() => {
					if (IME_DEBUG)
						console.log(
							"[IME Debug] activeElement after focus:",
	...
	if (imeInput) {
		setTimeout(() => {
			if (imeInput) {
				...
				if (IME_DEBUG)
					console.log(
						"[IME Debug] initial focus, activeElement:",
						document.activeElement?.tagName,
					);
			}
		}, 100);
...
async function handleKeyDown(event: KeyboardEvent): Promise<void> {
	...
	if (event.key === "Escape" && selectionManager && terminalRenderer) {
	...
	if (event.key === "c" && event.shiftKey && event.ctrlKey) {
		if (selectionManager && terminalState && clipboardManager) {
		...
		if (isComposing) {
			...
			if (text) {
				if (ptyClient) {
					...
					console.warn(
						"[EditContext] PTY client not ready, input dropped:",
						text,
					);
				}
			}
	...
	const onCompositionEnd = (event: any) => {
		...
		if (compositionText) {
			if (ptyClient) {
				...
				console.warn(
					"[EditContext] PTY client not ready, composition dropped:",
					compositionText,
				);
			}
		}
 ...
 */
function updateEditContextBounds(
	terminal: HTMLElement,
	ctx: EditContext,
): void {
	...
	ctx.updateSelectionBounds(new DOMRect(x, y, charSize.width, charSize.height));
	...
	if (textLength > 0) {
		...
		for (let i = 0; i < textLength; i++) {
			bounds.push(
				new DOMRect(x + i * charSize.width, y, charSize.width, charSize.height),
			);
		}
...
function setupIMEHandlers(
	...
	input.addEventListener("compositionend", async (event) => {
		...
		if (value === lastSentValue && now - lastSentTimestamp < 100) {
			if (IME_DEBUG)
				console.log("[IME Debug] compositionend: duplicate detected, skipping");
			input.value = "";
			updateCompositionView(view, "");
			return;
		}
		...
		try {
			...
			if (ptyClient) {
				...
				console.warn(
					"[IME Debug] PTY client not ready, composition dropped:",
					value,
				);
			}
		} catch (error) {
...
function setupMouseHandlers(container: HTMLElement): void {
	...
	const onMouseDown = (e: MouseEvent) => {
		if (!terminalState || !terminalRenderer || !selectionManager) return;

		// Check if mouse tracking is enabled
		const modes = terminalState.getModes();
		const isTracking = isMouseTrackingEnabled(modes.mouseTracking);

		// If mouse tracking is enabled and Shift is not pressed, send to PTY
		...
		selectionManager.startSelection(pos.col, pos.row);
		isSelecting = true;
		terminalRenderer.clearSelectionHighlight();
		e.preventDefault();
	};
	...
	const onMouseUp = (e: MouseEvent) => {
		if (isSelecting && selectionManager) {
			isSelecting = false;
			e.preventDefault();
			return;
		}
	...
	const onMouseMove = (e: MouseEvent) => {
		...
		if (isSelecting) {
			...
			selectionManager.updateSelection(pos.col, pos.row);

			// Update visual highlight
			const selection = selectionManager.getSelection();
			if (selection) {
				terminalRenderer.clearSelectionHighlight();
				terminalRenderer.renderSelection(selection);
			}
