...
import { PerformanceMonitor, ThroughputMeter, RenderTimer } from "./performance.ts";
...
describe("Terminal Action Processing Performance", () => {
  ...
  test("should process mixed actions efficiently", () => {
    ...
    for (let i = 0; i < 500; i++) {
      actions.push({ type: "Print", value: String.fromCharCode(65 + (i % 26)) });
      if (i % 10 === 0) {
        actions.push({
          type: "Csi",
  ...
  test("should handle rapid cursor movements", () => {
    ...
    for (let i = 0; i < 1000; i++) {
      actions.push({
        type: "Csi",
        value: { action: "CursorPosition", data: { row: (i % 24) + 1, col: (i % 80) + 1 } },
      });
...
describe("Throughput Benchmarks", () => {
  test("should estimate throughput for action processing", () => {
    ...
    console.log(`Processed ${result.bytesProcessed} bytes in ${result.durationMs.toFixed(2)}ms`);
    console.log(`Throughput: ${(result.bytesPerSecond / 1024 / 1024).toFixed(2)} MB/s`);
