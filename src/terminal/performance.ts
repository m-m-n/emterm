...
export class PerformanceMonitor {
	...
	getMetrics(): PerformanceMetrics {
		...
		const actionsPerSecond =
			elapsedSeconds > 0 ? this.totalActions / elapsedSeconds : 0;
		const throughputBps =
			elapsedSeconds > 0 ? this.totalBytes / elapsedSeconds : 0;
	...
	logMetrics(): void {
		...
		console.log(
			`Render Time: avg=${metrics.avgRenderTime.toFixed(2)}ms, max=${metrics.maxRenderTime.toFixed(2)}ms, min=${metrics.minRenderTime.toFixed(2)}ms`,
		);
		console.log(
			`Frames: ${metrics.renderCount}, Dropped: ${metrics.droppedFrames} (${(100 - metrics.frameSuccessRate).toFixed(1)}%)`,
		);
		console.log(
			`Actions: ${metrics.totalActions} total, ${metrics.actionsPerSecond.toFixed(0)}/sec`,
		);
		console.log(
			`Throughput: ${(metrics.throughputBps / 1024 / 1024).toFixed(2)} MB/s`,
		);
	}
}
...
export function checkFrameBudget(
	...
	if (durationMs > 16) {
		console.warn(
			`[Performance] ${context} exceeded frame budget: ${durationMs.toFixed(2)}ms (budget: 16ms)`,
		);
	}
}
...
export class ThroughputMeter {
	...
	stop(): {
		...
		const bytesPerSecond =
			durationSec > 0 ? this.bytesProcessed / durationSec : 0;
