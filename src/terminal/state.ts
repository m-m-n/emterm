...
export class TerminalState {
  ...
  takePendingResponse(): Uint8Array | null {
    ...
    const totalLength = this._pendingResponses.reduce((sum, r) => sum + r.length, 0);
  ...
  private handlePrint(char: string): void {
    ...
    if (code >= 0x20 && code < 0x7f && !this.wrapPending && this.activeCharSet === "G0" && this.g0CharSet === "Ascii") {
      const buffer = this.getActiveBuffer();
  ...
  private translateCharacter(char: string): string {
    const charSet = this.activeCharSet === "G0" ? this.g0CharSet : this.g1CharSet;
  ...
  private handleCsi(action: CsiAction): void {
    ...
    switch (action.action) {
      ...
      case "EraseCharacters":
        buffer.eraseCharacters(this.cursor.row, this.cursor.col, action.data || 1);
        break;
      ...
      case "InsertCharacters":
        buffer.insertCharacters(this.cursor.row, this.cursor.col, action.data || 1);
        break;

      case "DeleteCharacters":
        buffer.deleteCharacters(this.cursor.row, this.cursor.col, action.data || 1);
        break;
      ...
      case "SetScrollRegion": {
        ...
        const bottom =
          action.data.bottom === 0 ? this.rows - 1 : Math.max(0, action.data.bottom - 1);
