...
import type { TerminalAction, OscAction, CsiAction } from "../types/terminal.ts";
...
describe("TerminalState Phase 6", () => {
  ...
  describe("OSC - Hyperlink", () => {
    test("OSC 8 starts hyperlink", () => {
      ...
      const action: TerminalAction = {
        type: "Osc",
        value: { action: "Hyperlink", params: "id=1", uri: "https://example.com" },
      };
      state.processAction(action);
  ...
  describe("OSC - eMterm Extension", () => {
    test("EmtermExtension is handled without error", () => {
      ...
      state.processAction({
        type: "Osc",
        value: { action: "EmtermExtension", verb: "markdown", params: ["title", "body"] },
      });
