...
describe("BitmapCache", () => {
  describe("constructor", () => {
    ...
    test("creates cache with custom capacity", () => {
      const cache = new BitmapCache({ maxEntries: 50, maxMemoryBytes: 10 * 1024 * 1024 });
  ...
  describe("delete", () => {
    ...
    test("calls close on removed bitmap", async () => {
      ...
      const bitmap = { width: 10, height: 10, close: closeMock } as unknown as ImageBitmap;
  ...
  describe("LRU eviction", () => {
    test("evicts entries when capacity exceeded", async () => {
      const cache = new BitmapCache({ maxEntries: 3, maxMemoryBytes: 1024 * 1024 });
      ...
      const originalEntriesRemaining = [keys[0], keys[1], keys[2]].filter(k => cache.has(k)).length;
