...
| TS-6 | `hooks.json` structural check | Valid JSON; declares exactly `UserPromptSubmit`, `PostToolUse`, `PostToolUseFailure`, `Stop`, `PermissionRequest`, `Notification` in exec form; `PermissionRequest` carries `blocked` with no matcher; `${CLAUDE_PLUGIN_ROOT}`-prefixed commands; no absolute path, no `..`; `timeout` 3; no `StopFailure` or `SubagentStop` | Integration |
| TS-7 | `Notification` matcher behaviour | Matches `elicitation_dialog` / `agent_needs_input`; does not match `permission_prompt` (covered by `PermissionRequest`) / `idle_prompt` / `auth_success` / `elicitation_complete` / `elicitation_response` / `agent_completed` | Integration |
