...
| Replay-state probe (`dump_block`) | Determine the state that the client's replay establishes before the dump block (A6 resolution) | Input: the exact payload bytes that precede the dump block, their segments, and `current_dims`. Post: the scroll region, origin mode, cursor position, current SGR attributes and pending-wrap flag equal the state the client's Snapshot replay reaches at the dump block's first byte, after the trailing `current_dims` transition. This includes payloads whose segment resizes grow the screen (D8). The probe uses the same `term_core` replay entry point as the client. A `term_core` panic is contained and reported as a probe failure; it never unwinds into the caller. | task0001, task0002 |
| Dump block composer (`dump_block`) | Build the block that follows the pre-fix layout | Input: the probe state and the raw shadow dump. Post: (1) the block contains no DECSC / DECRC, no ED 3, no alt-screen toggle and no autowrap mode change. (2) Replaying the pre-dump payload + block at `current_dims` leaves every visible row equal to the shadow parser's row with the same index; no row is scrolled out or shifted. (3) Afterwards the scroll region, origin mode, cursor position, SGR and pending-wrap flag equal the probe state (D7), and the saved-cursor slot equals what the pre-dump replay left. (4) The block adds no rows to the client's scrollback history. | task0001, task0002 |
| `term_core` read-only observers | Let the probe and the tests read the scroll region, origin mode, cursor position, current SGR attributes, pending-wrap flag, saved-cursor slot and, where D7 needs it, a cell's character and attributes | Added only where no public accessor exists. Pure reads; `term_core` behaviour does not change. | task0001, task0002 |
...
- **Escape-sequence vocabulary of the snapshot's own additions**: scroll-region set/reset (DECSTBM), origin mode set/reset (DECSET / DECRST 6), absolute cursor positioning (CUP) and SGR only. The one other addition is the D7 re-print of the character the dump already drew at the end of a pending-wrap row. The additions never emit DECSC / DECRC, ED 3, alt-screen toggles or autowrap (DECAWM) changes.
...
### D7: Pending wrap is part of the restored cursor state (FR9; review round 1)

The absolute CUP in D4 step 3 clears a pending wrap. When the probe reports a pending wrap, the composer re-establishes it after the cursor restore and before the SGR restore. It re-prints, with its own attributes, the character the dump drew in the cell (or wide-cell pair) that ends at the last column of the cursor row. The re-printed cell equals what the dump drew there, so no visible cell changes, and the next printed character wraps as on a terminal that received the whole stream. When the flag is clear, the block is unchanged. This replaces the earlier Risk Assessment entry that accepted the lost pending wrap. Affected tasks: task0002 (changes the task0001 composer).

### D8: Probe history depth follows the client's grow-resize behaviour (FR9; review round 1)

`term_core` pulls rows back from history when a segment resize grows the screen. A zero-history probe then reaches a different cursor row than the client. The probe's scratch terminal therefore uses a bounded history depth large enough for the pull-backs to match. The contract is D3's probe-equality postcondition, checked against an oracle with 10,000 history lines on multi-segment payloads that grow. The D5 placement and cost rules are unchanged. Affected tasks: task0002 (changes the task0001 probe).
...
| The probe and the client reach different states through different replay entry points | Low | Medium | The probe uses the client's entry point; a test asserts that the probe state equals the `reset_and_replay_segments` state. For grow resizes, D8 sets the probe's history depth, and a test covers multi-segment grow payloads |
| A cursor in the pending-wrap state is restored without the pending flag | Low | Low | Resolved by D7 (review round 1); covered by TS-16 |
