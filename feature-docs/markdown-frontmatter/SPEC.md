...
- **Input Validation:** Delimiter scanning is bounded; parse errors are
  ...
  (FR6 path). The raw text shown in the error state is clamped to a
  display bound with a truncation marker (the DOM never receives
  unbounded front-matter text). Recursion depth capped at 128;
