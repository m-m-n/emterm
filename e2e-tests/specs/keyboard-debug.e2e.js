...
describe('Keyboard Debug Test', () => {
  ...
  it('should capture keyboard event logs', async () => {
    ...
    const logs = await browser.execute(() => {
      return window.consoleLogs.filter(log => log.includes('[handleKeyDown]'));
    });
  ...
  it('should test exit command with log capture', async () => {
    ...
    console.log('Session cleared:', initialSession !== null && finalSession === null);

    await browser.saveScreenshot('./screenshots/keyboard-debug-02-after-exit.png');
