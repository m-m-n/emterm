...
describe('Render Debug Test', () => {
  ...
  it('should track render after single keystroke', async () => {
    ...
    await browser.saveScreenshot('./screenshots/render-debug-02-after-keystroke.png');
  });
  ...
  it('should verify render with optimized mode disabled', async () => {
    ...
    await browser.saveScreenshot('./screenshots/render-debug-03-non-optimized.png');
  ...
  it('should test hash cache invalidation', async () => {
    ...
    await browser.saveScreenshot('./screenshots/render-debug-04-cache-cleared.png');
  });
  ...
  it('should trace full render cycle', async () => {
    ...
    await browser.execute(() => {
      ...
      renderer.renderLineOptimized = function(rowIndex, line) {
        ...
        console.log(`[TRACE] renderLineOptimized row=${rowIndex} hash=${contentHash.slice(0,50)}... lastHash=${lastHash?.slice(0,50)}... same=${lastHash === contentHash}`);
    ...
    const logs = await browser.execute(() => {
      return window.consoleLogs?.filter(log => log.includes('[TRACE]') || log.includes('[render]') || log.includes('[scheduleRender]')) || [];
    });
